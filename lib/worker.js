...
// Pre-initialize shared cache projection infrastructure
const sharedCache = workerData.sharedCache;
...
if (sharedCache) {
  ...
  const {
    install: fsPatchInstall,
  } = require('shared-memory-fs/adapters/fs-patch');
  const {
    install: requireHookInstall,
  } = require('shared-memory-fs/adapters/require-hook');
  const { vfsConfig: rawVfsConfig = {} } = workerData;
  const vfsConfig = new VfsConfig({
    defaults: {
      ...rawVfsConfig.defaults,
      hooks: { fs: true, require: true, import: false },
    },
    places: rawVfsConfig.places || {},
  });
  workerKernel = VFSKernel.fromSnapshot(sharedCache, vfsConfig, {
    appRoot: workerData.path,
  });
  fsPatchInstall(workerKernel);
  requireHookInstall(workerKernel);
  projectEntry = (entry) =>
    FilesystemCache.projectEntry(entry, workerKernel.segmentsMap);
}
application.workerKernel = workerKernel;
application.projectEntry = projectEntry;

// Send ACK after worker applies a cache update
const sendAck = sharedCache
...
const handlers = {
  'file-update': (msg) => {
    if (workerKernel) workerKernel.handleDelta(msg);
