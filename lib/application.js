...
class Application extends EventEmitter {
  ...
  async load({ invoke, sharedCache }) {
    if (!sharedCache) throw new Error('Shared cache snapshot is required');
    this.sharedCache = sharedCache;
    this.startWatch();
    this.createSandbox();
    this.sandbox.application.invoke = invoke;
    this.sandbox.application.emit('loading');
    this.applySharedCache(sharedCache, this.segmentsMap);
    await this.parallel([
      this.cert.load(),
      (async () => {
  ...
  applySharedCache(sharedCache, segmentsMap) {
    if (!sharedCache) throw new Error('Shared cache snapshot is required');
    if (!segmentsMap) {
      segmentsMap = new Map();
      for (const seg of sharedCache.segments) {
        segmentsMap.set(seg.id, seg.sab);
      }
    }
    const { indexes } = sharedCache;
    for (const name of Object.keys(indexes)) {
      const files = FileCache.project(indexes[name], segmentsMap);
      if (this[name]) this[name].setFiles(files);
    }
  ...
  startWatch() {
    ...
    const shared = this.sharedCache
      ? new Set(Object.keys(this.sharedCache.indexes))
      : new Set();
