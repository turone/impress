...
class Application extends EventEmitter {
  ...
  async load({ invoke, sharedCache }) {
    ...
    this.static.initServing(this.config);
    this.resources.initServing(this.config);
    this.startWatch();
    this.createSandbox();
  ...
  applySharedCache(sharedCache) {
    const { workerKernel, projectEntry } = this;
    const { filesystems } = sharedCache;
    for (const name of Object.keys(filesystems)) {
      const place = this[name];
      if (!place || typeof place.setFiles !== 'function') continue;
      if (workerKernel) {
        const kernelPlace = workerKernel.getPlace(name);
        if (kernelPlace) {
          place.setFiles(kernelPlace.files);
          place.kernelPlace = kernelPlace;
          continue;
        }
      }
