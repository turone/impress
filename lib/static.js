...
class Static extends Place {
  ...
  initCheckPath(opt) {
    const { ext, folders } = opt;
    const existFolders = (arrFolders) => {
      const folders = new Set(arrFolders);
      return (url) => folders.has(url.slice(0, url.indexOf('/', 1)));
    };
    const existExt = (arrExt) => {
      const ext = new Set(arrExt);
      return (url) => ext.has(metarhia.metautil.fileExt(url));
    };
    const existExtFolders = (arrExt, arrFolders) => {
      return (url) =>
        (existExt(arrExt)(url) || url.endsWith('/')) &&
        existFolders(arrFolders)(url);
    };
    const existRootFiles = (exist) => (url) => {
      if (!opt.rootFiles) return exist(url);
      const rootFiles = new Set(opt.rootFiles);
      return exist(url) || rootFiles.has(url);
    };
    if (!opt.disabledVirtualFolders) {
      if (ext && folders) {
        return existRootFiles(existExtFolders(ext, folders));
      }
      if (ext) return (url) => existExt(ext)(url);
      if (folders) return existRootFiles(existFolders(folders));
      return () => true;
    }
