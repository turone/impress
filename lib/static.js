...
class Static {
  ...
  async serve(url, transport) {
    ...
    if (exact && exact.data && exact.stat && !internal) {
      ...
      if (headers.range) {
        ...
        if (size > this.streamThreshold) {
          const readable = this.kernelPlace.createReadStream(filePath, {
            start,
            end,
          });
    ...
    const diskPath = filePath.endsWith('/')
      ? join(absPath, 'index.html')
      : absPath;
    if (diskPath.startsWith(this.path)) {
      const fsStat = await node.fsp.stat(diskPath).catch(() => null);
      if (fsStat && fsStat.isFile()) {
        const { size } = file?.stat ?? fsStat;
        const options = { size };
        let code = 200;
        const { headers } = transport.req;
        if (headers.range) {
        ...
        const readable = node.fs.createReadStream(diskPath, options);
