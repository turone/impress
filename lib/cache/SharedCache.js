...
const { StaticSource } = require('./StaticSource.js');
...
class SharedCache {
  constructor({ config, dir, console }) {
    ...
    const baseSegmentSize = sizeToBytes(sabConfig.baseSegmentSize || '64 mib');
    ...
    this.cache = new FileCache({ limit, baseSegmentSize, maxFileSize, reader });
  ...
  async initialize() {
    ...
    for (const placement of this.placements) {
      ...
      this.sources[placement.name] = new StaticSource(
        placement.name,
        appObj,
        opts,
      );
  ...
  watch(app) {
    ...
    for (const [name, source] of Object.entries(sources)) {
      sourcesByPath.push({ name, source, path: source.path });
    }

    const findSource = (filePath) => {
      for (const entry of sourcesByPath) {
        if (filePath.startsWith(entry.path)) return entry;
      }
      return null;
    };
    ...
    const processDelete = (name, source, filePath) => {
      ...
      const group = epoch.deletes[name] || (epoch.deletes[name] = []);
      for (const key of keys) {
        group.push(key);
        const old = cache.remove(name, key);
        if (old && old.kind === 'shared') epoch.oldEntries.push(old);
      }
    };
    ...
    this.watcher.on('delete', (filePath) => {
      const entry = findSource(filePath);
      if (entry) processDelete(entry.name, entry.source, filePath);
    });
  ...
  #tryCompact() {
    ...
    for (const name of Object.keys(byName)) {
      ...
      this.#trackUpdate(updateId, result.oldEntries);
    }
  }
