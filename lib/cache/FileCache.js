...
class FileCache {
  constructor(options = {}) {
    ...
    const baseSegmentSize =
      Math.ceil(maxFileSize / configured) * configured;
  ...
  free(entry) {
    if (!entry || entry.kind !== 'shared') return;
  ...
  static project(index, segmentsMap) {
    ...
    for (const [key, entry] of entries) {
      files.set(key, FileCache.projectEntry(entry, segmentsMap));
    }
    return files;
  }
  ...
  static projectEntry(entry, segmentsMap) {
    if (entry.kind === 'shared') {
      ...
      const data = Buffer.from(segmentsMap.get(segmentId), offset, length);
