...
When working in an area covered by `.github/instructions/*.instructions.md`, load the applicable file and follow it as the implementation-detail source of truth.
...
if multiple instruction files match, use ONLY the one whose `branch` frontmatter matches the current Git branch; ignore non-matching branch-scoped files
...
never rewrite large portions of Impress all at once
always ensure backward compatibility


FINAL RULE
`AGENTS.md` defines workflow only. Branch- and subsystem-specific architecture belongs in `.github/instructions/*.instructions.md` and must match the current branch code.
